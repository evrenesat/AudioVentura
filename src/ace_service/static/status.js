(() => {
  ...
  const outputHtml = (output) => {
    ...
    const download = output.download_url ? `<a href="${escapeHtml(output.download_url)}">Download</a>` : "";
    return `<div class="output-card"><div><strong>${title}</strong><span>${escapeHtml(output.size_label)} · ${escapeHtml(output.mime_type)}</span></div><button class="play-button" type="button" data-player-track data-player-title="${title}" data-player-src="${source}"${output.legacy ? ` src="${source}"` : ""} data-player-play aria-label="Play ${title}">▶ Play</button>${download}</div>`;
  };
  const refresh = async () => {
    if (stopped) return;
    try {
