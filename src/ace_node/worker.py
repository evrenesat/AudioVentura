...
from runpod_worker.runtime import WorkerInitializationError, compute_local_runtime_receipt
...
class AceStepNodeRuntime:
    ...
    def initialize(self) -> AceStepNodeRuntime:
        ...
        from runpod_worker.runtime import initialize_runtime

        self._runtime = initialize_runtime(self.settings.worker_hf_cache_root)
        return self
...
class NodeWorker:
    ...
    def health(self) -> dict[str, object]:
        ...
        return {
            "status": status if status in {"initializing", "ready", "failed"} else "failed",
            "error_code": failure,
