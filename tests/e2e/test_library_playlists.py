...
def test_library_search_sort_filter_and_custom_playlist_editing(
    ...
    expect(project_card.get_by_role("button", name="Keep offline")).to_be_visible()
    expect(project_card.get_by_role("button", name="Refresh")).to_be_visible()
    project_card.get_by_role("link", name="Beta ambient composition", exact=True).click()
    expect(e2e_page.get_by_label("Playlist title")).to_be_visible()
    expect(e2e_page.get_by_role("button", name="Rename")).to_be_visible()
    expect(e2e_page.get_by_role("button", name="Delete playlist")).to_be_visible()
    e2e_page.get_by_role("link", name="Playlists", exact=True).click()
    expect(e2e_page.get_by_role("heading", name="Playlists")).to_be_visible()
